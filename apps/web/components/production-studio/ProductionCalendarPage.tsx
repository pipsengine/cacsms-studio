...
export function ProductionCalendarPage(){
  ...
  return <section className={styles.page}>
    <header className={styles.header}><div><div className={styles.crumb}>Production Studio <span>/</span> Production Calendar</div><h1>Production Calendar</h1><p>Coordinate shoots, recordings, reviews, milestones, deadlines, and publishing schedules.</p></div><div className={styles.actions}><button onClick={refresh}><RefreshCw size={15} className={refreshing?styles.spin:""}/>Refresh</button><button className={styles.primary}><Plus size={16}/>New Event</button></div></header>
    <section className={styles.toolbar}><div className={styles.monthNav}><button aria-label="Previous month" onClick={()=>move(-1)}><ChevronLeft size={17}/></button><strong>{month.toLocaleString("en-US",{month:"long",year:"numeric"})}</strong><button aria-label="Next month" onClick={()=>move(1)}><ChevronRight size={17}/></button><button onClick={()=>setMonth(new Date(2026,6,1))}>Today</button></div><label><Filter size={15}/><select value={type} onChange={event=>setType(event.target.value as typeof type)}><option value="all">All event types</option>{Object.entries(typeLabels).map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label></section>
