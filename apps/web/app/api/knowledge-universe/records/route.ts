...
export async function POST(request:NextRequest) { try { return NextResponse.json(await createKnowledgeRecord(await request.json()),{status:201}); } catch(error) { return failure(error); } }
export async function DELETE(request:NextRequest) { try { const body=await request.json() as {ids?:string[]}; return NextResponse.json(await archiveKnowledgeRecords(Array.isArray(body.ids)?body.ids:[])); } catch(error) { return failure(error); } }
