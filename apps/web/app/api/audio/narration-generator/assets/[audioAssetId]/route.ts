...
import { loadNarrationAudioAsset } from "@/lib/narration-engine";
...
export async function GET(request: Request, context: { params: Promise<{ audioAssetId: string }> }) {
  ...
  try {
    ...
    const asset = await loadNarrationAudioAsset(audioAssetId);
    ...
    return Response.json(
      { message: error instanceof Error ? error.message : "The narration audio asset could not be loaded." },
      { status: 500 }
    );
  }
}
