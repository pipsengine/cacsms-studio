...
export default async function ContentIntelligenceRoute({params}:{params:Promise<{slug:string}>}){const {slug}=await params;if(slug==="intelligence-reports")return <IntelligenceReportsPage/>;if(slug==="competitor-intelligence")return <CompetitorIntelligencePage/>;if(slug==="citation-manager")return <CitationManagerPage/>;if(slug==="fact-verification")return <FactVerificationPage/>;if(slug==="curriculum-research")return <CurriculumResearchPage/>;if(slug==="research-workspace")return <ResearchWorkspacePage/>;if(slug==="source-library")return <SourceLibraryPage/>;if(slug==="audience-research")return <AudienceResearchPage/>;if(slug==="knowledge-extraction")return <KnowledgeExtractionPage/>;if(slug==="trend-intelligence")return <TrendIntelligencePage/>;if(slug==="content-gap-analysis")return <ContentGapAnalysisPage/>;if(slug==="knowledge-base")return <KnowledgeBasePage/>;const page=contentIntelligencePages[slug];if(!page)notFound();return <ContentIntelligencePage page={page}/>}
