...
const DEFAULT_PROVIDER = "local-png-renderer";
const DEFAULT_MODEL = "CACSMS Local Visual Worker";
const MAX_RETRIES = 3;
  ...
  const result = await pool
    ...
    .query<ProductionRow>(`
      ...
      WHERE CONVERT(nvarchar(36), p.WorkspaceId) = @workspace
        AND p.Status NOT IN (N'archived', N'published')
        AND p.Stage IN (N'storyboard', N'visual-generation', N'assembly')
      ORDER BY
        CASE p.Stage
          WHEN N'visual-generation' THEN 0
...
export async function runImageGenerationScheduler(): Promise<ImageGeneratorPayload> {
  ...
  let job = await latestJob(pool, row.ProductionId);
  let variants = await productionVariants(pool, row.ProductionId);
  ...
  await patchJob(pool, job.ImageGenerationJobId, {
    ...
    storageResult: "The worker is generating real PNG bytes.",
    modelResponse: JSON.stringify({ provider: DEFAULT_PROVIDER, model: DEFAULT_MODEL, providerJobId, assignedAt: new Date().toISOString() })
  });
  await patchVariant(pool, activeVariant.ImageGenerationVariantId, {
    state: "Generating",
    retryCount: job.RetryCount,
  ...
  await patchJob(pool, job.ImageGenerationJobId, {
    ...
    storageResult: `Generated ${rendered.bytes.length} bytes and preparing the filesystem write.`,
    modelResponse: JSON.stringify({ provider: DEFAULT_PROVIDER, model: DEFAULT_MODEL, providerJobId, averageLuma: rendered.averageLuma, byteLength: rendered.bytes.length })
  });
